...
extern "C" void hooc_hvm_arc_retain_if_managed(uint64_t obj);
extern "C" void hooc_hvm_arc_release_if_managed(uint64_t obj);
...
std::vector<RuntimeSymbolContract> buildRuntimeSymbols() {
    return {
        ...
        {"hoo_string_to_upper", reinterpret_cast<void*>(&hoo_string_to_upper)},
        ...
        {"hoo_exception_runtime", reinterpret_cast<void*>(&hoo_exception_runtime)},
        {"hoo_exception_clear", reinterpret_cast<void*>(&hoo_exception_clear)},
        {"hooc_hvm_sys_alloc", reinterpret_cast<void*>(&hooc_hvm_sys_alloc)},
        {"hooc_hvm_sys_retain", reinterpret_cast<void*>(&hooc_hvm_sys_retain)},
        {"hooc_hvm_sys_release", reinterpret_cast<void*>(&hooc_hvm_sys_release)},
        {"hooc_hvm_sys_refcount", reinterpret_cast<void*>(&hooc_hvm_sys_refcount)},
        {"hooc_hvm_sys_typeid", reinterpret_cast<void*>(&hooc_hvm_sys_typeid)},
        {"hooc_hvm_sys_exception_runtime", reinterpret_cast<void*>(&hooc_hvm_sys_exception_runtime)},
        {"hooc_hvm_arc_retain_if_managed", reinterpret_cast<void*>(&hooc_hvm_arc_retain_if_managed)},
        {"hooc_hvm_arc_release_if_managed", reinterpret_cast<void*>(&hooc_hvm_arc_release_if_managed)},
    };
}
...
bool HVMJIT::loadSource(const std::string& sourcePath) {
    ...
    if (source->empty()) {
        setError(ErrorPhase::Parse, ErrorCode::ParseFailed,
                 "Source file is empty: " + sourcePath, "", "", sourcePath);
        return false;
    }

    const std::string moduleName = fs::path(sourcePath).stem().string();
    auto module = sourceCompiler_->compileToHVM(moduleName, *source);
    if (!module) {
        setError(ErrorPhase::Parse, ErrorCode::ParseFailed,
...
bool HVMJIT::bootstrapRuntimeModules() {
    ...
    if (runtimeSymbols.empty() ||
        runtimeSymbols[0].addr == nullptr ||
        runtimeSymbols[1].addr == nullptr ||
        runtimeSymbols[2].addr == nullptr) {
...
const hvm::Symbol* HVMJIT::findFunctionSymbol(const hvm::HOModule& module, const std::string& functionName) const {
    ...
    for (const auto& sym : symbols) {
        if (isFunc(sym) && sym.name == functionName) {
            return &sym;
        }
    }
    ...
    for (const auto& sym : symbols) {
        ...
        if (sym.name == "_F_" + functionName + "_v" || sym.name == functionName) {
            return &sym;
        }
    }
    return nullptr;
        ...
        switch (ins->getOpcode()) {
            ...
            case hvm::Opcode::CALL: {
                ...
                const std::string calleeName = [&]() -> std::string {
                    for (const auto& s : module->getSymbols()) {
                        if (s.type == hvm::Symbol::STT_FUNC && s.value == target) return s.name;
                    }
                    return "";
                }();
                if (calleeName.empty()) {
...
bool HVMJIT::initializeModules() {
    ...
    for (const auto& base : ordered) {
        ...
        const auto* initSym = findFunctionSymbol(*mod, "_F_module_init_v");
        if (initSym) {
            if (executeFunction(mod, initSym->name, state) == -1 && !lastError_.empty()) {
                lastError_ = "Module init failed for " + mod->getName() + ": " + lastError_;
                return false;
            }
        }
    }
        ...
        while (!work.empty()) {
            ...
            while (ipc < textSize) {
                ...
                if (op == hvm::Opcode::RET) {
                    ...
                    if (fnNameIt == functionNameByOffset_[hvmModule.getName()].end() || !fnMap.count(fnNameIt->second)) {
                        builder.CreateRet(builder.getInt64(-1));
                        break;
                    }
                    writeReg(o.rd, builder.getInt64(nextPc));
                    auto* rv = builder.CreateCall(fnMap[fnNameIt->second], {stateArg});
                    writeReg(1, rv);
                    ...
                    auto* sw = builder.CreateSwitch(builder.getInt64(static_cast<int64_t>(o.imm15)), syscallErr, 6);
...
int64_t HVMJIT::run(const std::string& entryPoint) {
    ...
    state.memory = memory_.data();
    state.regs[31] = static_cast<int64_t>(memory_.size() - 16);
    return executeFunction(primary, entryPoint, state);
}
