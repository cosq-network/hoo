...
- `hoo_character_from_codepoint(int64_t)`: Creates a character from a Unicode codepoint.
- `hoo_character_from_utf8(const char*, int64_t)`: Creates a character from a raw UTF-8 sequence.
- `hoo_character_codepoint(ch)`: Retrieves the Unicode codepoint.
...
- `hoo_string_from_any(val, type_id)`: Generic intrinsic for converting any primitive or object to a string.
- `hoo_string_from_object(obj)`: Converts a managed object to a string.
