...
- `Uuid.from_bytes(bytes)` — Create from 16 raw bytes.

## 2. Conversion

- `u.format()` — Convert to canonical string (free with `Uuid.free_string`).
- `u.to_bytes(out_16)` — Write 16 raw bytes into buffer. Returns 1 on success.

## 3. Properties & Comparison

- `u.is_nil()` — Returns 1 if nil.
...
Strings allocated by `u.format()` must be freed with `Uuid.free_string(str)`.
