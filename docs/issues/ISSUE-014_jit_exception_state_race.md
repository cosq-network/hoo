...
- **Resolution**: `gCurrentExceptionByState` now has its own mutex. Shadow-handler stack operations and exception-state operations have separate ownership boundaries.
- **Concurrent Regression**: `ConcurrentExceptionThrowCatchIsSafeAcrossThreadsAndStates` (`tests/hvm/HVMJITLoaderTest.cpp`) runs N worker threads each owning their own `HVMJIT` instance and looping through push/throw/catch sequences, exercising the shared maps under contention.
- **Lock-Ordering Invariant**: `gShadowHandlersMu` is always acquired **before** `gCurrentExceptionMu` (`src/hvm/HVMJIT.cpp` lines 559-570 and 592-595); neither is ever acquired while holding the inverse. This ordering is documented here as the single source of truth.
