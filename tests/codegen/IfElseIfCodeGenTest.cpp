...
TEST_F(IfElseIfCodeGenTest, IfElseIfComplexConditionAnd) {
    std::string code = R"(
        func:int64 test(x: int64, y: int64) {
            if (x > 0 && y > 0) {
                return 1;
            } else {
                if (x < 0 && y < 0) {
...
TEST_F(IfElseIfCodeGenTest, IfElseIfMixedTypes) {
    std::string code = R"(
        func:int64 test(intVal: int64, floatVal: double, boolVal: bool) {
            if (intVal > 0) {
                return 1;
            } else {
                if (floatVal > 2.0) {
...
TEST_F(IfElseIfCodeGenTest, IfElseIfBooleanVariable) {
    std::string code = R"(
        func:int64 test(flag1: bool, flag2: bool) {
            if (flag1 && flag2) {
                return 1;
            } else {
                if (flag1) {
